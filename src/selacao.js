...
export default function Sel(props){
    ...
    useEffect(() => {
        async function getpg1(){
         try{
            const promessa=await axios.get('https://rpg-daniel.herokuapp.com/selecao',{headers: {
                authorization : token
             }})
    ...
    async function b(){
        try {
            alert('oi')
            if(num===10){
            const resposta = await axios.post(`https://rpg-daniel.herokuapp.com/adicionar`,cards,{headers: {
                authorization : token
             }})
